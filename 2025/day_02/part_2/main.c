...
int is_number_repeating(uint64_t number){
    ...
    char substring[string_length + 1];
    ...
    for(int i = 1; i <= string_length / 2; i++){
        // if the length of the substring does not evenly fit
        // into the string, there's no point in trying.
        if(string_length % i != 0) continue;
        
        is_repeating = 1;

        // create the substring
        for(int ii = 0; ii < i; ii++){
            substring[ii] = string[ii];
        }

        // printf("\n(%s)", substring);

        // check the substring
        for(int ii = i; ii <= string_length - i; ii += i){
            
            for(int iii = 0; iii < i; iii++){
                if(substring[iii] != string[ii+iii]) is_repeating = 0;
            }
        }
